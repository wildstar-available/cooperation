    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                            ...
                            for c in content:
                                if c.get("type") == "output_text":
                                    ...
                                    if "## 2. 推荐治疗方案" in full_text:
                                        ...
                                        diagnosis = full_text[:1000]
                                        treatment = full_text[1000:]
                                    else:
                                        diagnosis = full_text[:1000]
                                        treatment = full_text[1000:] if len(full_text) > 1000 else ""

                if diagnosis or treatment:
                    return {
                        "diagnosis": diagnosis[:1000] if diagnosis else "分析完成",
                ...
                return {
                    "diagnosis": "AI 服务暂时不可用",
                    "treatment": f"请稍后重试（状态码: {response.status_code}）"
                }
    except Exception:
        return {
            "diagnosis": "AI 服务暂时不可用",
            "treatment": "请检查网络连接后重试"
        }
